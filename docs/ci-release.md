...
PRs and main pushes validate and package `mujoco-runtime` on GitHub-hosted Ubuntu 24.04. Version tag pushes publish a release. For existing tags, run **CI and Release** on **main** with the exact tag as input. Published releases are never overwritten and tags are never moved.
