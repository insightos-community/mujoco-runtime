"""Run native Windows physics and offscreen rendering; preserve failure evidence."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--physics-only", action="store_true", help="Do not qualify graphics on runners without a qualified graphics driver"
    )
    ...
    try:
        if platform.system() != "Windows" or platform.machine().lower() not in {"amd64", "x86_64"}:
            raise RuntimeError("This validation requires native Windows x64")
        ...
        report.update(mujoco=mujoco.__version__, numpy=np.__version__, backend="glfw")
        ...
        if args.physics_only:
            report["rendering"] = "not-tested: physical Windows GPU qualification required"
            return
